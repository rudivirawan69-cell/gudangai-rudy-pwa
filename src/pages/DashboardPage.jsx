...
import { getPendingQueue, getTransactionHistory, getStatusPO } from '../data/api';
import { DIVISIONS } from '../data/master';
import {
  AlertTriangle, ShieldCheck, WifiOff, ChevronRight, RefreshCw,
  Snowflake, BarChart3, FileText
} from 'lucide-react';
...
function statusPOColor(status) {
  if (status === 'Selesai') return { bg: 'bg-emerald-50', text: 'text-emerald-700', bar: 'bg-emerald-500', border: 'border-emerald-100' };
  if (status === 'Sebagian') return { bg: 'bg-amber-50', text: 'text-amber-700', bar: 'bg-amber-400', border: 'border-amber-100' };
  return { bg: 'bg-slate-50', text: 'text-slate-600', bar: 'bg-slate-300', border: 'border-slate-100' };
}

function StatusPOCard({ data, loading, error, onRefresh }) {
  if (loading) {
    return (
      <div className="card p-3.5 animate-slide-up" style={{ animationDelay: '40ms', animationFillMode: 'both' }}>
        <div className="flex items-center gap-2 mb-3">
          <FileText className="w-4 h-4 text-violet-600" />
          <h3 className="text-sm font-semibold text-slate-800">Status Purchase Order</h3>
        </div>
        <div className="space-y-2"><div className="skeleton h-10 rounded-xl" /><div className="skeleton h-16 rounded-xl" /></div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="card p-3.5 animate-slide-up border border-amber-100" style={{ animationDelay: '40ms', animationFillMode: 'both' }}>
        ...
        <p className="text-[11px] text-amber-700 bg-amber-50 rounded-lg px-2.5 py-2">{error}</p>
      </div>
    );
  }
  if (!data || !data.success) {
    return (
      <div className="card p-3.5 animate-slide-up" style={{ animationDelay: '40ms', animationFillMode: 'both' }}>
        ...
        <p className="text-[11px] text-slate-400 text-center py-3">Belum ada data PO aktif</p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-3.5 animate-slide-up" style={{ animationDelay: '40ms', animationFillMode: 'both' }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <button type="button" onClick={onRefresh} className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 active:scale-95 shrink-0" aria-label="Refresh PO">
          <RefreshCw className="w-3.5 h-3.5" />
        </button>
      </div>
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-xl bg-slate-50 border border-slate-100 px-2 py-2 text-center">
          <p className="text-[9px] uppercase tracking-wider text-slate-500 font-medium">Menunggu</p>
          <p className="text-lg font-bold text-slate-700 tabular-nums leading-tight">{menunggu}</p>
        </div>
        <div className="rounded-xl bg-amber-50 border border-amber-100 px-2 py-2 text-center">
          <p className="text-[9px] uppercase tracking-wider text-amber-700 font-medium">Sebagian</p>
          <p className="text-lg font-bold text-amber-700 tabular-nums leading-tight">{sebagian}</p>
        </div>
        <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-2 py-2 text-center">
          <p className="text-[9px] uppercase tracking-wider text-emerald-700 font-medium">Selesai</p>
          <p className="text-lg font-bold text-emerald-700 tabular-nums leading-tight">{selesai}</p>
        </div>
      </div>
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-slate-500 font-medium">Progress kedatangan</span>
          <span className="text-[11px] font-bold text-violet-700 tabular-nums">{progressPct}%</span>
        </div>
        <div className="h-2.5 w-full rounded-full bg-slate-100 overflow-hidden flex">
          <div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${totalItem ? Math.round((selesai / totalItem) * 100) : 0}%` }} />
          <div className="h-full bg-amber-400 transition-all duration-500" style={{ width: `${totalItem ? Math.round((sebagian / totalItem) * 100) : 0}%` }} />
        </div>
      ...
      {items.length > 0 && (
        <div className="space-y-1.5 max-h-52 overflow-y-auto">
          {items.slice(0, 8).map((it, idx) => {
            ...
            return (
              <div key={it.itemNo || idx} className={`rounded-xl border px-2.5 py-2 ${col.border} ${col.bg}`}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-semibold text-slate-800 truncate">
                      <span className="text-slate-400 font-medium mr-1">#{String(it.itemNo || idx + 1).padStart(2, '0')}</span>
                      {it.nama}
                    </p>
                    <p className="text-[9px] text-slate-400 mt-0.5">{it.size ? `${it.size} · ` : ''}{it.satuan || 'Pack'}{it.tglRencana ? ` · Rencana ${it.tglRencana}` : ''}</p>
                  </div>
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${col.text} bg-white/80`}>{st}</span>
                </div>
                <div className="flex items-center gap-2">
          ...
          {items.length > 8 && <p className="text-[10px] text-slate-400 text-center pt-1">+{items.length - 8} item lainnya</p>}
        </div>
      )}
    </div>
  );
...
export default function DashboardPage({ onNavigate }) {
  ...
  const loadStatusPO = useCallback(async () => {
    ...
    try {
      ...
      if (res && res.success) { setPoData(res); setPoError(null); }
      else { setPoData(res); setPoError(res?.error || 'Gagal memuat Status PO'); }
    } catch (err) {
  ...
  useEffect(() => {
    loadStatusPO();
    const id = setInterval(() => { if (navigator.onLine) loadStatusPO(); }, 90000);
  ...
  const pieSegs = [{ n: aman, c: '#10b981' }, { n: waspada, c: '#f59e0b' }, { n: kritis, c: '#ef4444' }];
  let pieOff = 0;
    ...
    <div className="pb-3 animate-fade-in space-y-3">
        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`inline-flex items-center gap-1 text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${isOnline ? 'bg-emerald-400/20 text-emerald-200' : 'bg-red-400/20 text-red-200'}`}>
              ...
              {lastRefresh && (
                <p className="text-[10px] text-blue-200/50 mt-0.5">Stok disinkron {lastRefresh.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</p>
              )}
            </div>
            <button type="button" onClick={() => { stockCV.refresh(); stockPT.refresh(); loadStatusPO(); }} disabled={loading}
          ...
          <div className="mt-3 rounded-xl bg-black/15 border border-white/10 p-2.5">
            <div className="flex items-center justify-between mb-1 px-0.5">
              <p className="text-[11px] font-semibold text-white/90 flex items-center gap-1"><BarChart3 className="w-3.5 h-3.5 text-cyan-300" /> Pergerakan 7 Hari</p>
            ...
            <p className="text-[9px] text-blue-100/45 text-center mt-0.5">
              ...
              {deltaPct !== 0 && <span className={deltaPct > 0 ? ' text-orange-300' : ' text-emerald-300'}> ({deltaPct > 0 ? '+' : ''}{deltaPct}% vs kemarin)</span>}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card p-3.5 animate-slide-up" style={{ animationDelay: '180ms', animationFillMode: 'both' }}>
        <div className="flex items-center justify-between mb-2.5">
          ...
          <button type="button" onClick={() => onNavigate && onNavigate('stok')} className="text-[11px] text-cyan-700 font-medium flex items-center gap-0.5">
        ...
        {criticalItems.length === 0 ? (
          ...
          <ul className="space-y-1.5">
            {criticalItems.map((item) => {
              ...
              return (
                <li key={`${item.entity}-${item.kode}`} className="flex items-center justify-between gap-2 rounded-xl bg-slate-50 px-2.5 py-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-semibold text-slate-800 truncate">{item.nama}</p>
                    <p className="text-[9px] text-slate-400">{item.entity} · {item.kode}</p>
                  </div>
