...
function DivisionStatusBars({ items }) {
  ...
  const resetView = () => setTilt({ x: 8, y: -6 });
  ...
  const MiniDonut = ({ r }) => {
    ...
    const kritisPct = Math.round((r.kritis / total) * 100);
    return (
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
          <circle cx={size / 2} cy={size / 2} r={rad} fill="none" stroke="#e2e8f0" strokeWidth={stroke} />
          {r.aman > 0 && <circle cx={size / 2} cy={size / 2} r={rad} fill="none" stroke="#10b981" strokeWidth={stroke} strokeDasharray={`${segA} ${c - segA}`} strokeDashoffset={0} strokeLinecap="butt" />}
          {r.waspada > 0 && <circle cx={size / 2} cy={size / 2} r={rad} fill="none" stroke="#f59e0b" strokeWidth={stroke} strokeDasharray={`${segW} ${c - segW}`} strokeDashoffset={-segA} strokeLinecap="butt" />}
          {r.kritis > 0 && <circle cx={size / 2} cy={size / 2} r={rad} fill="none" stroke="#ef4444" strokeWidth={stroke} strokeDasharray={`${segK} ${c - segK}`} strokeDashoffset={-(segA + segW)} strokeLinecap="butt" />}
        </svg>
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className={`text-[10px] font-black tabular-nums ${kritisPct >= 40 ? 'text-red-600' : 'text-slate-700'}`}>{kritisPct}%</span>
        </div>
      </div>
    );
  };
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-[10px] text-slate-400 font-medium">Geser untuk memiringkan · 3D</p>
        <button type="button" onClick={resetView} className="text-[10px] font-semibold px-2 py-1 rounded-lg bg-slate-100 text-slate-600 border border-slate-200 active:scale-95">Reset View</button>
      </div>
      <div className="select-none touch-none" style={{ perspective: '700px', cursor: dragging ? 'grabbing' : 'grab' }}
        onMouseDown={onPointerDown} onMouseMove={onPointerMove} onMouseUp={onPointerUp} onMouseLeave={onPointerUp}
        onTouchStart={onPointerDown} onTouchMove={onPointerMove} onTouchEnd={onPointerUp}>
        <div className="space-y-2.5 transition-transform duration-75" style={{ transform: `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`, transformStyle: 'preserve-3d' }}>
          {rows.map((r, idx) => {
            const kritisPct = r.total ? Math.round((r.kritis / r.total) * 100) : 0;
            return (
              <div key={r.divisi} className="flex items-center gap-3 rounded-2xl bg-gradient-to-r from-white to-slate-50 border border-slate-100 shadow-sm p-2.5 animate-slide-up"
                style={{ animationDelay: `${idx * 40}ms`, animationFillMode: 'both', transform: `translateZ(${8 - idx * 1.5}px)`, boxShadow: '0 4px 14px -2px rgba(15,23,42,0.08)' }}>
                <MiniDonut r={r} />
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden flex">
                    ...
                    <div className="h-full bg-red-500" style={{ width: `${(r.kritis / r.total) * 100}%` }} />
                  </div>
                  <div className="flex gap-3 mt-1 text-[9px] text-slate-500">
                    ...
                    {kritisPct > 0 && <span className="ml-auto text-red-600 font-semibold">{kritisPct}% kritis</span>}
                  </div>
                </div>
              </div>
            );
...
function StatusPOCard({ data, loading, error, onRefresh }) {
  ...
  const [dragging, setDragging] = useState(false);
  const startRef = useRef({ x: 0, y: 0, tx: 12, ty: -8 });
  const resetView = () => setTilt({ x: 12, y: -8 });
  const onPointerDown = (e) => {
    setDragging(true);
    const cx = e.clientX ?? e.touches?.[0]?.clientX ?? 0;
    const cy = e.clientY ?? e.touches?.[0]?.clientY ?? 0;
  ...
  const onPointerMove = (e) => {
    ...
    const cx = e.clientX ?? e.touches?.[0]?.clientX ?? 0;
    const cy = e.clientY ?? e.touches?.[0]?.clientY ?? 0;
    const dx = (cx - startRef.current.x) * 0.25;
    const dy = (cy - startRef.current.y) * 0.25;
    setTilt({
      x: Math.max(-25, Math.min(25, startRef.current.tx + dy)),
      y: Math.max(-30, Math.min(30, startRef.current.ty + dx)),
    });
  };
  const onPointerUp = () => setDragging(false);
  if (loading) {
    return (
      <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-4 animate-slide-up">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div><h3 className="text-sm font-semibold text-slate-800">Status PO Aktif 3D</h3><p className="text-[10px] text-slate-400">Memuat data PO…</p></div>
        </div>
        <div className="space-y-2"><div className="skeleton h-3 rounded-full" /><div className="skeleton h-36 rounded-xl" /></div>
      </div>
    );
  }
  if (error) {
      ...
      <div className="rounded-2xl bg-white border border-amber-100 shadow-sm p-4 animate-slide-up">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full bg-violet-100 flex items-center justify-center"><FileText className="w-4 h-4 text-violet-600" /></div>
            <h3 className="text-sm font-semibold text-slate-800">Status PO Aktif 3D</h3>
          </div>
    ...
    return (
      <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-4 animate-slide-up">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full bg-violet-100 flex items-center justify-center"><FileText className="w-4 h-4 text-violet-600" /></div>
            <div><h3 className="text-sm font-semibold text-slate-800">Status PO Aktif 3D</h3><p className="text-[10px] text-slate-400">Belum ada data PO aktif</p></div>
          </div>
          <button type="button" onClick={onRefresh} className="text-[11px] text-cyan-700 font-medium">Muat ulang</button>
        </div>
  ...
  const pctSelesai = Math.round((selesai / totalSeg) * 1000) / 10;
  const pctSebagian = Math.round((sebagian / totalSeg) * 1000) / 10;
  const pctMenunggu = Math.round((menunggu / totalSeg) * 1000) / 10;
  const size = 160;
  const stroke = 22;
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-slate-800 leading-tight">Status PO Aktif 3D</h3>
            <p className="text-[10px] text-slate-400 truncate">{noPO ? `No. PO ${noPO}` : 'Tidak ada No. PO aktif'} · {totalItem} item · Interactive</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <button type="button" onClick={resetView} className="text-[10px] font-semibold px-2 py-1 rounded-lg bg-slate-100 text-slate-600 border border-slate-200 active:scale-95">Reset View</button>
          <button type="button" onClick={onRefresh} className="w-8 h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 active:scale-95" aria-label="Refresh PO"><RefreshCw className="w-3.5 h-3.5" /></button>
        </div>
      </div>
      <div className="relative mx-auto mb-4 select-none touch-none" style={{ width: size + 24, height: size + 24, perspective: '600px', cursor: dragging ? 'grabbing' : 'grab' }}
        onMouseDown={onPointerDown} onMouseMove={onPointerMove} onMouseUp={onPointerUp} onMouseLeave={onPointerUp}
        onTouchStart={onPointerDown} onTouchMove={onPointerMove} onTouchEnd={onPointerUp}>
        <div className="absolute inset-0 flex items-center justify-center transition-transform duration-75" style={{ transform: `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`, transformStyle: 'preserve-3d' }}>
          <div className="absolute rounded-full bg-slate-900/10 blur-md" style={{ width: size * 0.85, height: size * 0.22, bottom: 8, transform: 'translateZ(-20px)' }} />
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90 drop-shadow-lg" style={{ filter: 'drop-shadow(0 8px 16px rgba(15,23,42,0.18))' }}>
            ...
            {sebagian > 0 && <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#f59e0b" strokeWidth={stroke} strokeDasharray={`${segSebagian} ${c - segSebagian}`} strokeDashoffset={-offsetSebagian} strokeLinecap="butt" />}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-3xl font-black text-slate-800 tabular-nums leading-none">{progressPct}%</span>
            <span className="text-[10px] text-slate-400 mt-1 font-semibold tracking-wide">Progress</span>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-2.5 text-center">
          <span className="text-[10px] text-emerald-600 font-bold block">Complete</span>
          <span className="text-sm font-black text-emerald-700 tabular-nums">{pctSelesai}%</span>
          <span className="text-[9px] text-emerald-600/80 block mt-0.5">{selesai} item</span>
        </div>
        <div className="rounded-xl bg-amber-50 border border-amber-100 p-2.5 text-center">
          <span className="text-[10px] text-amber-600 font-bold block">Progress</span>
          <span className="text-sm font-black text-amber-700 tabular-nums">{pctSebagian}%</span>
          <span className="text-[9px] text-amber-600/80 block mt-0.5">{sebagian} item</span>
        </div>
        <div className="rounded-xl bg-slate-50 border border-slate-200 p-2.5 text-center">
          ...
          <span className="text-sm font-black text-slate-700 tabular-nums">{pctMenunggu}%</span>
          <span className="text-[9px] text-slate-500/80 block mt-0.5">{menunggu} item</span>
        </div>
      </div>
      {totalItem === 0 && (
                ...
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="min-w-0 flex-1">
                    <p className="text-[12px] font-semibold text-slate-800 truncate">
                      <span className="text-slate-400 font-medium mr-1.5">#{String(it.itemNo || idx + 1).padStart(2, '0')}</span>{it.nama}
                    </p>
    ...
    <div className="pb-6 animate-fade-in space-y-3">
      ...
      <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-4">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-8 h-8 rounded-full bg-cyan-50 flex items-center justify-center"><Package className="w-4 h-4 text-cyan-600" /></div>
          <div>
            <h3 className="text-sm font-semibold text-slate-800">Status Stok Per Divisi 3D</h3>
            <p className="text-[10px] text-slate-400">Aman · Waspada · Kritis · Interactive</p>
          </div>
        </div>
