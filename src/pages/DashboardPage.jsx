...
export default function DashboardPage() {
  ...
  else if (hour >= 15 && hour < 18) { greeting = 'Selamat sore'; reminder = 'Apakah pengeluaran hari ini sudah diinput? Jangan lupa input data real-time ya.'; }

  return (
    <div className="pb-6 animate-fade-in space-y-3">
      <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-3.5">
        <p className="text-sm font-bold text-slate-800">{greeting}, {firstName}</p>
        <p className="text-[11px] font-bold text-pink-500 leading-snug mt-0.5">{reminder}</p>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        <div className="rounded-2xl border border-slate-100 bg-white shadow-sm px-2 py-3.5 flex flex-col items-center gap-1.5">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center bg-cyan-50 text-cyan-600">
            <Package className="w-5 h-5" />
          </div>
          <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Total SKU</p>
          <p className="text-xl font-bold tabular-nums text-slate-800 leading-none">{stats.total}</p>
          <p className="text-[9px] text-slate-400">{stats.totalUnit.toLocaleString('id-ID')} unit</p>
        </div>
        <div className="rounded-2xl border border-emerald-100 bg-white shadow-sm px-2 py-3.5 flex flex-col items-center gap-1.5 ring-2 ring-emerald-400/30">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center bg-emerald-500 text-white">
            <span className="text-lg font-bold">✓</span>
          </div>
          <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wide">Aman</p>
          <p className="text-xl font-bold tabular-nums text-emerald-700 leading-none">{stats.aman}</p>
          <p className="text-[9px] text-slate-400">{stats.total ? Math.round((stats.aman / stats.total) * 100) : 0}% SKU</p>
        </div>
        <div className="rounded-2xl border border-red-100 bg-white shadow-sm px-2 py-3.5 flex flex-col items-center gap-1.5">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center bg-red-500 text-white">
            <AlertTriangle className="w-5 h-5" />
          </div>
          <p className="text-[10px] font-semibold text-red-600 uppercase tracking-wide">Kritis</p>
          <p className="text-xl font-bold tabular-nums text-red-600 leading-none">{stats.kritis}</p>
          <p className="text-[9px] text-slate-400">{stats.waspada} waspada</p>
        </div>
      </div>
