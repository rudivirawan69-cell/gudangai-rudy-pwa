  ...
  return (
    <div className="rounded-xl bg-white border border-gray-100 shadow-sm p-3">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <button type="button" aria-label={`Hapus ${item.nama}`} onClick={() => onRemove(item.id)} className="text-red-400 p-1.5">
          <Trash2 className="w-4 h-4" />
        </button>
      </div>
      <div className="flex items-center gap-2">
        <button type="button" aria-label="Kurangi" onClick={() => onUpdate(item.id, Math.max(0, item.qty - 1))} className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center">
        ...
        <input
          ...
          className="w-20 text-center text-sm font-bold border border-gray-200 rounded-lg py-2"
        />
        <button type="button" aria-label="Tambah" onClick={() => onUpdate(item.id, item.qty + 1)} className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center">
    ...
    <div className="pb-28 animate-fade-in">
      <div className="flex items-center justify-between mb-1">
        <div>
          ...
          <p className="text-xs text-gray-400">Rekomendasi PO langsung tampil dan dapat diedit sebelum dikirim.</p>
        </div>
        <button type="button" onClick={regenerate} className="p-2 rounded-lg bg-white border border-gray-100 text-gray-500" aria-label="Generate ulang">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>

      <div className="grid grid-cols-2 gap-2 mb-3 mt-3">
        <button type="button" onClick={() => switchTab('cs')} className={`rounded-xl px-3 py-3 flex items-center gap-2 border-2 ${tab === 'cs' ? 'border-cyan-500 bg-cyan-50' : 'border-gray-100 bg-white'}`}>
        ...
        <button type="button" onClick={() => switchTab('produksi')} className={`rounded-xl px-3 py-3 flex items-center gap-2 border-2 ${tab === 'produksi' ? 'border-orange-500 bg-orange-50' : 'border-gray-100 bg-white'}`}>
      ...
      {phase === 'review' && (
        <>
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-gray-500">{loading ? 'Memuat stok...' : `${draftItems.filter((i) => Number(i.qty) > 0).length} item aktif`}</p>
            <button type="button" onClick={regenerate} className="text-xs text-cyan-700 font-semibold">Generate ulang</button>
          </div>
          {!loading && draftItems.length === 0 ? (
            ...
            <div className="space-y-2">
              {draftItems.map((item) => <ItemCard key={item.id} item={item} onUpdate={updateQty} onRemove={removeItem} />)}
            </div>
          )}
          <button type="button" disabled={finalRows.length === 0} onClick={() => setPhase('final')} className="w-full mt-4 rounded-xl bg-slate-900 text-white py-3 text-sm font-bold disabled:opacity-40">
      ...
      {phase === 'final' && (
        <>
          <div className="flex items-center justify-between mb-3">
            <div><p className="text-sm font-bold text-gray-800">Hasil PO Siap Dikirim</p><p className="text-[11px] text-gray-400">Tanggal {dateParts(0).long} · Kedatangan {dateParts(2).short}</p></div>
            <button type="button" onClick={() => setPhase('review')} className="px-3 py-2 rounded-lg bg-white border border-gray-200 text-xs font-semibold"><ArrowLeft className="w-4 h-4 inline mr-1" />Edit</button>
          </div>
          <div className="overflow-x-auto rounded-xl border border-gray-100 bg-white">
            <table className="w-full text-xs">
              <thead><tr className="bg-gray-50 text-gray-500"><th className="px-3 py-2 text-left">No</th><th className="px-3 py-2 text-left">Nama Barang</th><th className="px-3 py-2 text-right">CV</th><th className="px-3 py-2 text-right">PT</th><th className="px-3 py-2 text-right">Total</th></tr></thead>
              <tbody>{finalRows.map((r, idx) => <tr key={`${r.kode}-${idx}`} className="border-t border-gray-50"><td className="px-3 py-2">{idx + 1}</td><td className="px-3 py-2 font-medium">{r.nama}<div className="text-[10px] text-gray-400">{r.kode} · {r.satuan}</div></td><td className="px-3 py-2 text-right">{r.poCV || '-'}</td><td className="px-3 py-2 text-right">{r.poPT || '-'}</td><td className="px-3 py-2 text-right font-bold">{r.total}</td></tr>)}</tbody>
            </table>
          </div>
          <button type="button" disabled={submitting} onClick={handleSubmit} className="w-full mt-4 rounded-xl bg-slate-900 text-white py-3 text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50">
            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} {submitting ? 'Mengirim...' : 'Kirim PO'}
          </button>
        </>
      )}
      ...
      {phase === 'saved' && (
        <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-8 text-center">
          ...
          <button type="button" onClick={resetSaved} className="mt-5 px-4 py-2 rounded-lg bg-gray-100 text-sm font-semibold">Kembali</button>
        </div>
      )}

      {submitMsg && phase !== 'saved' && <p className="mt-3 text-xs text-center text-red-500">{submitMsg}</p>}
    </div>
  );
}
