...
function MenuRow({ icon: Icon, iconBg, title, subtitle, onClick, right, danger }) {
  return (
    <button type="button" onClick={onClick}
      className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:scale-[0.98] transition-transform rounded-2xl">
      <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${iconBg || 'bg-slate-100 text-slate-600'}`}>
...
export default function SettingsPage({ onOpenSyncQueue }) {
  ...
  const handleSync = () => {
    if (typeof onOpenSyncQueue === 'function') {
      onOpenSyncQueue();
    }
  };
    ...
    <div className="pb-6 animate-fade-in space-y-3">
      <div className="card p-4 flex items-center gap-3 bg-gradient-to-r from-cyan-50/90 via-white to-violet-50/80 border border-cyan-100/60">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-slate-500">SPV Gudang · Cold Storage NG69</p>
          <p className="text-[10px] text-cyan-700 mt-0.5 font-semibold">GudangAI RUDY</p>
        </div>
      </div>
      ...
      <div className="card">
        <MenuRow icon={Link2} iconBg="bg-cyan-100 text-cyan-700" title="Koneksi Google Sheets" subtitle={connSubtitle}
      ...
      <div className="card">
        <MenuRow icon={Cloud} iconBg="bg-violet-100 text-violet-600" title="Mode Offline & Sync" subtitle={pendingCount ? `${pendingCount} transaksi pending` : '0 — antrian kosong'} onClick={() => setPanel(panel === 'sync' ? null : 'sync')} />
      </div>
      <div className="card">
      ...
      {panel === 'koneksi' && (
        <div className="card p-4 space-y-3">
          ...
          {connMessage && (
            <p className={`text-xs rounded-lg px-3 py-2 ${connStatus === 'ok' ? 'bg-emerald-50 text-emerald-700' : connStatus === 'fail' ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-600'}`}>
              {connMessage}
            </p>
          )}
        </div>
      ...
      {panel === 'sync' && (
        <div className="card p-4 space-y-3">
          <p className="text-xs font-semibold text-slate-600">Antrian offline</p>
          <p className="text-sm text-slate-700">{pendingCount} transaksi pending</p>
          <div className="flex gap-2">
            <button type="button" onClick={handleSync} className="flex-1 py-2.5 rounded-xl bg-[#0b2a55] text-white text-sm font-semibold flex items-center justify-center gap-1.5">
              <RefreshCw className="w-4 h-4" /> Buka Antrian
            </button>
            <button type="button" onClick={handleClearSynced} className="py-2.5 px-3 rounded-xl bg-slate-100 text-slate-600" title="Hapus antrian">
      ...
      <div className="card border border-red-100/80">
        <MenuRow icon={LogOut} iconBg="bg-red-100 text-red-600" title="Keluar" subtitle="Login ulang dengan PIN" onClick={() => setShowLogout(true)} danger />
      </div>
      {showLogout && (
