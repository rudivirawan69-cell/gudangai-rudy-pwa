                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {r.status === 'unmatched' && (
                          <div className="mt-1.5 space-y-1.5">
                            ...
                            {(r.searchHits || []).map((c) => (
                              <button key={c.kode} type="button" onClick={() => pickCandidate(idx, c)}
                                className="block w-full text-left text-xs px-2 py-1.5 rounded-lg bg-gray-50 text-gray-700">
                                {c.kode} — {c.nama}
                              </button>
                            ))}
                            <button type="button" onClick={() => skipReviewItem(idx)}
      ...
      {cart.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-800">Keranjang ({cart.length})</h3>
            <button type="button" onClick={() => setCart([])} className="text-xs text-red-500 flex items-center gap-1">
              <Trash2 className="w-3.5 h-3.5" /> Kosongkan
            </button>
          </div>
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {cart.map((c, idx) => (
              <div key={c.kode + idx} className="bg-white rounded-xl border border-gray-100 px-3 py-2.5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold text-gray-800 truncate">{c.nama}</p>
                    <p className="text-[10px] text-gray-400">{c.kode} · {c.satuan}</p>
                  </div>
                  <button type="button" onClick={() => removeFromCart(idx)} className="text-gray-300 p-0.5">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
                <div className="flex items-center gap-2 mt-1.5">
                  <button type="button" onClick={() => updateQty(idx, -1)} className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center">
                    <Minus className="w-3.5 h-3.5 text-gray-500" />
                  </button>
                  <input type="number" step="0.01" value={c.qty} onChange={(e) => setQtyValue(idx, e.target.value)}
                    className="w-16 text-center text-sm font-bold border border-gray-200 rounded-lg py-1" />
                  <button type="button" onClick={() => updateQty(idx, 1)} className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center">
                    <Plus className="w-3.5 h-3.5 text-gray-500" />
                  </button>
                  <input value={c.keterangan} onChange={(e) => updateKet(idx, e.target.value)}
                    placeholder="Ket (opsional)" className="flex-1 text-[11px] px-2 py-1 bg-gray-50 border border-gray-100 rounded-lg" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="fixed bottom-[4.25rem] left-0 right-0 z-40 px-3 pointer-events-none">
            <div className="max-w-lg mx-auto pointer-events-auto">
              <button type="button" onClick={submitCart} disabled={submitting || !cart.length}
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-sm font-bold flex items-center justify-center gap-2 shadow-xl disabled:opacity-50">
