...
export default function InputPage() {
  ...
  const mergePdfIntoCart = useCallback((matched, amb, un) => {
    setCart((prev) => {
      ...
      for (const r of un) {
        flags.push({ kode: '', nama: r.nameFromPdf || 'Tidak dikenali', satuan: '', qty: r.qty || 1, keterangan: '', status: 'flag', fromPdf: true, nameFromPdf: r.nameFromPdf, candidates: searchMaster(entity, r.nameFromPdf || '').slice(0, 4), note: 'Validasi manual diperlukan.' });
      }
  ...
  const onFile = async (e) => {
    ...
    try {
      ...
      setStatusBanner(matched.length ? ('Validasi ' + acc + '% · ' + matched.length + ' cocok' + ((un.length || amb.length) ? (' · ' + (un.length + amb.length) + ' perlu cek') : '')) : 'Tidak ada nama yang cocok di master/alias.');
  ...
  const submitCart = () => {
    ...
    submitFn(entity, snapshot, { tanggal })
      .then((res) => {
        ...
        if (res.success) pushNotification({ type: 'ok', title: 'Kirim berhasil', body: (res.written || snapshot.length) + ' item ' + (txType === 'masuk' ? 'Masuk' : txType === 'rusak' ? 'Rusak' : 'Keluar') + ' ' + entity + ' tercatat.' });
        else pushNotification({ type: res.offline ? 'warn' : 'err', title: res.offline ? 'Disimpan antrian offline' : 'Kirim bermasalah', body: res.error || 'Cek Atur → Offline & Sync' });
      })
  ...
  return (
    <div className="pb-28 animate-fade-in space-y-3">
      ...
      {showNotif && (
        <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-3 max-h-52 overflow-y-auto">
          {notifs.length === 0 ? <p className="text-[11px] text-slate-400 text-center py-3">Belum ada notifikasi</p> : (
            <ul className="space-y-2">{notifs.slice(0, 12).map((n) => (
      ...
      <div className="flex gap-2 p-1 bg-slate-100/80 rounded-2xl">
        {['CV', 'PT'].map((e) => (
          <button key={e} type="button" onClick={() => { setEntity(e); setHits([]); setQuery(''); }} className={`flex-1 py-2.5 rounded-xl text-sm font-bold ${entity === e ? 'bg-gradient-to-r from-cyan-500 to-violet-600 text-white shadow-md' : 'text-slate-500'}`}>{e}</button>
        ))}
      </div>
      ...
      {accuracy && (
        <div className="rounded-2xl bg-gradient-to-r from-violet-50 to-cyan-50 border border-violet-100 px-3 py-2.5 text-[12px] text-slate-700 shadow-sm">
          ...
          {accuracy.skipped ? <span className="text-amber-600 font-medium"> · perlu cek {accuracy.skipped}</span> : null}
        </div>
      )}
      ...
      <div className="rounded-2xl bg-white border border-slate-100 p-3 space-y-2 shadow-sm">
        ...
        {hits.length > 0 && (
          <ul className="space-y-1 max-h-40 overflow-y-auto">{hits.map((h) => (
            <li key={h.kode}>
              <button type="button" onClick={() => addToCart(h, 1)} className="w-full text-left px-2.5 py-2 rounded-xl hover:bg-cyan-50 text-[12px] font-medium text-slate-700 flex items-center justify-between gap-2">
                <span className="truncate">{h.nama}</span><span className="text-[10px] text-slate-400 shrink-0">{h.kode}</span>
              </button>
            </li>
          ))}</ul>
        )}
      ...
      {cart.length > 0 && (
        <div className="rounded-2xl bg-white border-2 border-cyan-200/80 p-3 space-y-2.5 shadow-md ring-1 ring-cyan-100/60">
          <div className="flex items-center justify-between">
            <p className="text-[12px] font-bold text-slate-800 tracking-tight">
              Keranjang ({cart.length})
              {readyCount > 0 && <span className="ml-1.5 text-emerald-600 font-semibold">· {readyCount} siap</span>}
              {flagCount > 0 && <span className="ml-1.5 text-amber-600 font-semibold">· {flagCount} perlu cek</span>}
            </p>
          </div>
          <div className="space-y-2.5 max-h-96 overflow-y-auto">
            {cart.map((c, idx) => (
              <div key={(c.kode || c.nameFromPdf || 'x') + '-' + idx} className={`rounded-xl border-2 p-2.5 shadow-sm ${
                c.status === 'flag' ? 'bg-gradient-to-br from-amber-50 to-orange-50 border-amber-300'
                : c.status === 'fallback' ? 'bg-gradient-to-br from-amber-50/80 to-yellow-50 border-amber-200'
                : 'bg-gradient-to-br from-emerald-50/90 to-cyan-50/50 border-emerald-200'
              }`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      {c.status === 'ok' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />}
                      {(c.status === 'flag' || c.status === 'fallback') && <AlertTriangle className="w-3.5 h-3.5 text-amber-600 shrink-0" />}
                      <p className="text-[12px] font-bold text-slate-900 truncate">{c.nama}</p>
                    </div>
                  ...
                  <button type="button" onClick={() => removeFromCart(idx)} className="p-1 text-slate-400 hover:text-rose-500" aria-label="Hapus"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
                {c.status === 'flag' && c.candidates?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">{c.candidates.map((cand) => (
                    <button key={cand.kode} type="button" onClick={() => resolveFlag(idx, cand)} className="text-[10px] px-2 py-1 rounded-lg bg-white border border-teal-200 text-teal-700 font-semibold">Pilih {cand.kode} · {cand.nama}</button>
                  ))}</div>
                )}
                {c.status === 'flag' && (
                  <button type="button" onClick={() => { setPendingManualIdx(idx); setQuery(c.nameFromPdf || c.nama); setHits(searchMaster(entity, c.nameFromPdf || c.nama).slice(0, 8)); }} className="mt-2 text-[10px] px-2 py-1 rounded-lg bg-teal-50 border border-teal-200 text-teal-700 font-semibold">Cari & pilih master</button>
                )}
                <div className="mt-2.5 space-y-2">
                  <div>
                    <p className="text-[9px] uppercase tracking-wider text-slate-400 font-semibold mb-1">Qty</p>
                    <div className="flex items-center gap-1.5 max-w-[11rem]">
                      <button type="button" onClick={() => updateQty(idx, -1)} className="w-9 h-9 shrink-0 rounded-xl bg-white border border-slate-200 flex items-center justify-center"><Minus className="w-3.5 h-3.5 text-slate-500" /></button>
                      <input type="number" step="0.01" value={c.qty} onChange={(e) => setQtyValue(idx, e.target.value)} className="flex-1 min-w-0 text-center text-sm font-bold border border-slate-200 rounded-xl py-2 bg-white" />
                      <button type="button" onClick={() => updateQty(idx, 1)} className="w-9 h-9 shrink-0 rounded-xl bg-white border border-slate-200 flex items-center justify-center"><Plus className="w-3.5 h-3.5 text-slate-500" /></button>
                    </div>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-[12px] px-3 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-400/30"
                    />
                  </div>
                </div>
              </div>
      ...
      {cart.length === 0 && !busy && <p className="text-center text-[12px] text-slate-400 py-4">Unggah PDF, scan QR, suara, atau cari nama barang.</p>}
      {cart.length > 0 && (
        <div className="fixed bottom-[4.25rem] left-0 right-0 z-40 px-3 pointer-events-none">
          <div className="max-w-lg mx-auto pointer-events-auto">
            <button type="button" onClick={submitCart} disabled={submitting || readyCount === 0} className={`w-full py-3.5 rounded-2xl bg-gradient-to-r ${submitGradient} text-white text-sm font-bold flex items-center justify-center gap-2 shadow-xl disabled:opacity-70`}>
              {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
              {submitting ? 'Mengirim…' : readyCount === 0 ? `Selesaikan ${flagCount} item dulu` : `Kirim ${readyCount} item · ${typeLabel} (${entity})`}
            </button>
          </div>
        </div>
      )}
