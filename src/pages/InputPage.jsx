...
export default function InputPage() {
  ...
  const updateQty = (idx, delta) => {
    setCart((prev) => prev.map((c, i) => i === idx ? { ...c, qty: Math.max(0.5, +(c.qty + delta).toFixed(2)) } : c));
  };
  ...
  const dismissPdfSheet = () => { setPdfResults([]); setError(''); };
            ...
            <div key={c.kode + '-' + idx} className="bg-white rounded-xl border border-gray-100 p-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => updateQty(idx, -1)} className="p-1 rounded-lg bg-gray-100 text-gray-600"><Minus className="w-3.5 h-3.5" /></button>
                  <span className="w-8 text-center text-sm font-bold text-gray-800">{c.qty}</span>
                  <button type="button" onClick={() => updateQty(idx, 1)} className="p-1 rounded-lg bg-gray-100 text-gray-600"><Plus className="w-3.5 h-3.5" /></button>
                </div>
