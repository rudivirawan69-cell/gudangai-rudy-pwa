...
export default function InputPage() {
  ...
  const onPdfPick = async (e) => {
    ...
    try {
      ...
      if (res && res.ok === false) {
        setStatusBanner(res.error || 'Gagal baca PDF');
        setBusy(false);
        return;
      }
      const text = res?.text || '';
      ...
      if (!text.trim()) {
        setStatusBanner('PDF tidak berisi teks terbaca.');
        setBusy(false);
        return;
      }
  ...
  const onImgPick = async (e) => {
    ...
    try {
      ...
      if (res && res.ok === false) {
        setStatusBanner(res.error || 'Gagal OCR foto');
        setBusy(false);
        return;
      }
      const text = res?.text || '';
      const lines = res?.lines || [];
      if (!text.trim()) {
        setStatusBanner('Foto tidak berisi teks terbaca. Coba foto lebih jelas.');
        setBusy(false);
        return;
      }
      await runValidationPipeline(text, lines, 'Foto');
    } catch (err) {
      setStatusBanner(err.message || 'Gagal OCR foto');
      setBusy(false);
    }
  };
  ...
  const startScan = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setStatusBanner('Kamera tidak didukung di perangkat ini.');
      return;
    }
    scanAbortRef.current = false;
    setScanning(true);
    setStatusBanner('Menyalakan kamera… arahkan ke barcode/QR');
    let stream = null;
    try {
      ...
      if (value) {
        ...
        if (found[0]) {
          addToCart(found[0], 1);
          setStatusBanner('Barcode cocok: ' + found[0].nama);
        } else {
          setQuery(String(value));
          setStatusBanner('Barcode tidak cocok master: ' + value);
        }
  ...
  const stopScan = () => {
    ...
    setScanning(false);
    setStatusBanner('Scan dibatalkan');
  };

  const startVoice = () => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) {
      setStatusBanner('Suara tidak didukung. Pakai Chrome Android / Safari terbaru.');
      return;
    }
    try {
      const rec = new SR();
      ...
      setStatusBanner('🎤 Mendengarkan… sebutkan nama barang');
      setBusy(true);
      rec.onresult = (ev) => {
        const t = (ev.results?.[0]?.[0]?.transcript || '').trim();
        setBusy(false);
        if (!t) {
          setStatusBanner('Tidak ada suara terdeteksi. Coba lagi.');
          return;
        }
        setQuery(t);
        ...
        if (found[0]) {
          addToCart(found[0], 1);
          setStatusBanner('Suara cocok: ' + found[0].nama);
        } else if (found.length > 1) {
          setHits(found.slice(0, 12));
          setStatusBanner('Suara: ' + t + ' — pilih dari daftar');
        } else {
          setStatusBanner('Suara: "' + t + '" — tidak cocok master. Ketik manual.');
        }
      };
      rec.onerror = (ev) => {
        setBusy(false);
      ...
      setBusy(false);
      setStatusBanner(err?.message || 'Gagal memulai rekaman suara');
    }
  };
  ...
  return (
    <div className="pb-24 space-y-3 text-white">
      ...
      {scanning && (
        <div className="rounded-xl overflow-hidden border border-cyan-400/40 bg-black">
          ...
          <div className="flex items-center justify-between px-3 py-2 bg-black/80">
            ...
            <button type="button" onClick={stopScan} className="text-xs font-bold text-rose-300 px-2 py-1 rounded-lg bg-white/10">
              Stop
            </button>
          </div>
        </div>
      ...
      {showNotif && (
        <div className="fixed inset-0 z-50 bg-black/60" onClick={() => setShowNotif(false)}>
          <div className="absolute top-16 right-3 left-3 max-w-sm ml-auto rounded-2xl bg-slate-900 border border-white/15 p-3 max-h-80 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <p className="text-sm font-bold text-white mb-2">Notifikasi</p>
            {notifs.length === 0 && <p className="text-xs text-white/40">Belum ada notifikasi</p>}
