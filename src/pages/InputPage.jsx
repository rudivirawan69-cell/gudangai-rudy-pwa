...
export default function InputPage() {
  ...
  const resolveValidationRow = (row, item) => {
    ...
    setValidationRows((prev) => prev.map((x) => x.key === row.key ? { ...x, status: 'MATCH_MANUAL', kode: item.kode, nama: item.nama, displayName: item.nama, candidates: [] } : x));
    setAccuracy((prev) => {
  ...
  const onFile = async (e) => {
    ...
    try {
      ...
      // Acuan D: tampilkan Kode | Nama Barang (bersih) | Qty | Status
      // nameFromPdf disimpan untuk audit/manual; tampilan utama memakai nama master
      setValidationRows([
        ...matched.map((r, i) => ({
          key: `m-${i}-${r.kode}`,
          nameFromPdf: r.nameFromPdf,
          displayName: r.nama || r.nameFromPdf,
          qty: r.qty,
          status: r.fallback ? 'FALLBACK' : 'MATCH',
          kode: r.kode,
          nama: r.nama,
          candidates: [],
          note: r.fallback || '',
        ...
        ...amb.map((r, i) => ({
          key: `a-${i}-${r.nameFromPdf}`,
          nameFromPdf: r.nameFromPdf,
          displayName: r.nameFromPdf,
          qty: r.qty,
          status: 'AMBIGU',
          kode: r.kode || '',
          nama: r.nama || '',
        ...
        ...un.map((r, i) => ({
          ...
          kode: '',
          nama: '',
          candidates: searchMaster(useEntity, r.nameFromPdf).slice(0, 4),
    ...
    <div className="pb-28 animate-fade-in space-y-3">
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {validationRows.map((row) => (
              <div key={row.key} className="rounded-xl bg-slate-50 border border-slate-100 p-2.5">
                <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-[11px] font-semibold text-slate-700 truncate">{row.displayName || row.nama || row.nameFromPdf}</p><p className="text-[10px] text-slate-400">Qty {row.qty} · {row.kode || 'Belum ada kode'}</p></div><span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${row.status === 'MATCH' ? 'bg-emerald-100 text-emerald-700' : row.status === 'FALLBACK' ? 'bg-amber-100 text-amber-700' : row.status === 'MATCH_MANUAL' ? 'bg-teal-100 text-teal-700' : 'bg-rose-100 text-rose-700'}`}>{row.status}</span></div>
      ...
      {cart.length > 0 && (
        <div className="rounded-2xl bg-white border border-slate-100 p-3 space-y-2.5 shadow-sm">
          <p className="text-[11px] font-semibold text-slate-600">Keranjang ({cart.length})</p>
          <div className="space-y-2.5 max-h-80 overflow-y-auto">
            {cart.map((c, idx) => (
              <div key={c.kode + '-' + idx} className="rounded-xl bg-slate-50 border border-slate-100 p-2.5">
