...
export default function InputPage() {
  ...
  const mergePdfIntoCart = useCallback((matched, amb, un) => {
    setCart((prev) => {
      ...
      // un (bukan master) DIABAIKAN total — tidak masuk keranjang
    ...
    <div className="pb-28 animate-fade-in space-y-3">
      ...
      {cart.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-slate-600 px-1">Keranjang ({cart.length}) · siap {readyCount}{flagCount ? ` · perlu pilih ${flagCount}` : ''}</p>
          {cart.map((c, idx) => (
            <div key={idx} className={`rounded-2xl border p-3 ${c.status === 'flag' ? 'bg-amber-50/80 border-amber-200' : c.status === 'fallback' ? 'bg-violet-50/60 border-violet-100' : 'bg-white border-slate-100'} shadow-sm`}>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <input value={c.keterangan || ''} onChange={(e) => updateKet(idx, e.target.value)} placeholder="Keterangan" className="flex-1 text-xs border border-slate-100 rounded-lg px-2 py-1.5" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="fixed bottom-16 left-0 right-0 px-4 z-20">
        <button type="button" onClick={submitCart} disabled={submitting || readyCount === 0} className={`w-full max-w-lg mx-auto py-3.5 rounded-2xl bg-gradient-to-r ${submitGradient} text-white text-sm font-bold flex items-center justify-center gap-2 shadow-lg disabled:opacity-50`}>
