...
export default function InputPage() {
  ...
  const typeConfig = {
    masuk: { label: 'Masuk', icon: PackagePlus, color: 'bg-emerald-100 text-emerald-700', activeColor: 'bg-emerald-500 text-white', modeClass: 'mode-active-masuk' },
    keluar: { label: 'Keluar', icon: PackageMinus, color: 'bg-orange-100 text-orange-700', activeColor: 'bg-orange-500 text-white', modeClass: 'mode-active-keluar' },
    rusak: { label: 'Rusak', icon: AlertTriangle, color: 'bg-red-100 text-red-700', activeColor: 'bg-red-500 text-white', modeClass: 'mode-active-rusak' },
  };

  const addItem = (item, extra = {}) => {
    setItems((prev) => {
      const exist = prev.find((x) => x.kode === item.kode);
      if (exist) {
        return prev.map((x) => x.kode === item.kode ? { ...x, qty: (x.qty || 0) + (extra.qty || 1), ...extra } : x);
      }
      return [...prev, { ...item, qty: extra.qty || 1, keterangan: extra.keterangan || '', source: extra.source || 'manual', matchType: extra.matchType || '', voiceRaw: extra.voiceRaw || '' }];
    });
    setShowSearch(false);
    setSearch('');
  };
  ...
  const runValidateFromText = useCallback((raw, entityOverride) => {
    const lines = parseLinesFromText(raw || '');
    const res = validateItems(lines, entityOverride || entity);
  ...
  const handleValidateText = () => {
    ...
    runValidateFromText(uploadText);
  };
  ...
  const handleFileText = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setVoiceError('Gagal baca file: ' + (err.message || err));
      setVoiceHint('');
    } finally {
      setPdfBusy(false);
  ...
  const handleSubmit = async () => {
    if (submitLock.current || !items.length || submitting) return;
    ...
    try {
      ...
      let res;
      if (type === 'masuk') res = await submitBarangMasuk(entity, payload);
      else if (type === 'keluar') res = await submitBarangKeluar(entity, payload);
      else res = await submitBarangRusak(entity, payload);
      if (res?.ok || res?.success) {
        try { await saveToHistory({ type, entity, items: payload, at: new Date().toISOString() }); } catch (_) {}
        setResult({ ok: true, message: res.message || `Berhasil kirim ${items.length} item` });
        setItems([]);
        setConfirmedFromUpload(false);
      } else {
        setResult({ ok: false, message: res?.message || res?.error || 'Gagal kirim' });
      }
    } catch (err) {
  ...
  return (
    <div className="pb-36 pt-3 min-h-[60vh]">
      ...
      {result && (
        <div className={`mb-4 rounded-xl px-4 py-3 text-sm flex items-start gap-2 ${result.ok ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
          {result.ok ? <CheckCircle className="w-5 h-5 shrink-0 mt-0.5" /> : <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />}
          <p>{result.message}</p>
        </div>
      )}
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-2.5 text-center">
                    ...
                    <p className="text-[10px] text-emerald-700 font-medium">Cocok</p>
                  </div>
                  <div className="rounded-xl bg-amber-50 border border-amber-100 p-2.5 text-center">
                    <p className="text-lg font-bold text-amber-600">{(validationResult.ambiguous || []).length}</p>
                    <p className="text-[10px] text-amber-700 font-medium">Ambigu</p>
                  </div>
                  <div className="rounded-xl bg-red-50 border border-red-100 p-2.5 text-center">
                    ...
                    <p className="text-[10px] text-red-700 font-medium">Tidak cocok</p>
                  </div>
                </div>
                  ...
                  <div key={'a' + i} className="bg-amber-50 rounded-xl px-4 py-3 mb-2 border border-amber-200">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <button type="button"
                        ...
                        className="text-[11px] text-amber-600 underline">Lewati (tidak cocok)</button>
                    </div>
                  </div>
                ))}
