    ...
    <div className="pb-28 animate-fade-in space-y-4">
            ...
            <div className="space-y-3">
              ...
              {pdfSummary.matched > 0 && pdfSummary.ambiguous === 0 && (
                <button type="button" onClick={addReviewedToCart}
                  ...
                  <Plus className="w-4 h-4" /> Tambah {pdfSummary.matched} item ke keranjang
                </button>
              )}
      ...
      {cart.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-bold text-gray-700 flex items-center gap-1.5">
            <PackagePlus className="w-4 h-4 text-emerald-600" /> Keranjang ({cart.length} item)
          </h3>
          {cart.map((c, idx) => (
            <div key={c.kode + '-' + idx} className="bg-white rounded-xl border border-gray-100 p-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => updateQty(idx, -1)} className="p-1.5 rounded-lg bg-gray-100 text-gray-600"><Minus className="w-3.5 h-3.5" /></button>
                  <input
                    type="number"
                    inputMode="decimal"
                    min="0.01"
                    ...
                    className="w-16 text-center text-sm font-bold text-gray-800 bg-gray-50 border border-gray-200 rounded-lg py-1 focus:outline-none focus:border-cyan-400"
                  />
                  <button type="button" onClick={() => updateQty(idx, 1)} className="p-1.5 rounded-lg bg-gray-100 text-gray-600"><Plus className="w-3.5 h-3.5" /></button>
                </div>
                <button type="button" onClick={() => removeFromCart(idx)} className="p-1 text-gray-400"><Trash2 className="w-4 h-4" /></button>
              </div>
      ...
      {submitMsg && (
        <div className={`text-xs px-3 py-2 rounded-lg ${
          ...
          {submitMsg.success
            ? (submitMsg.offline ? 'Disimpan offline — akan sync saat online' : `${submitMsg.written || cart.length} item terkirim ke server${submitMsg.batch ? ' (batch)' : ''}`)
