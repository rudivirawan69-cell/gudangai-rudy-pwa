    ...
    submitFn(entity, snapshot, { tanggal })
      .then((res) => {
        ...
        if (res.success) {
          ...
          // api.js sudah kirim "Verifikasi antrian" — jangan override dengan keterangan gagal
        } else if (res.offline) {
      ...
      {cart.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between px-1">
            <p className="text-xs font-bold text-slate-600">Keranjang ({cart.length})</p>
            {flagCount > 0 && <span className="text-[10px] text-amber-600 font-medium">{flagCount} perlu cek</span>}
          </div>
          {cart.map((c, idx) => (
            <div key={idx} className={`rounded-2xl border p-3 ${c.status === 'flag' ? 'bg-amber-50/80 border-amber-200' : c.status === 'fallback' ? 'bg-violet-50/60 border-violet-100' : 'bg-white border-slate-100'} shadow-sm`}>
              <div className="flex items-start gap-2">
                ...
                <button type="button" onClick={() => removeFromCart(idx)} className="p-1.5 text-slate-400 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
              </div>
              {c.status === 'flag' && c.candidates?.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {c.candidates.map((cand) => (
                    <button key={cand.kode} type="button" onClick={() => resolveFlag(idx, cand)} className="text-[10px] px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 font-medium">{cand.nama}</button>
                  ))}
                </div>
              )}
              <div className="mt-2.5 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => updateQty(idx, -1)} className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center"><Minus className="w-3.5 h-3.5" /></button>
                  <input type="number" step="0.01" value={c.qty} onChange={(e) => setQtyValue(idx, e.target.value)} className="w-16 text-center text-sm font-bold border border-slate-200 rounded-lg py-1" />
                  <button type="button" onClick={() => updateQty(idx, 1)} className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center"><Plus className="w-3.5 h-3.5" /></button>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-400 w-12">Ket</span>
                  <input type="text" value={c.keterangan} onChange={(e) => updateKet(idx, e.target.value)} placeholder="Keterangan (opsional)" className="flex-1 text-xs border border-slate-200 rounded-lg px-2 py-1.5" />
                </div>
              </div>
            </div>
          ))}
