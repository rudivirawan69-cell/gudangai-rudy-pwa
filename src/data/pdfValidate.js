 ...
 * Acuan ketat validasi PDF (12 Sep 2026): strip NO urut, hanya nama+TOTAL qty
 */
...
/**
 * Detect and merge 2-column PDF layout (qty rows upper, names lower after KETERANGAN).
 */
...
// Acuan ketat: buang header, label kolom, nama outlet, NO urut
const HEADER_RE = /^(no\.?|tanggal|nama\s*barang|jumlah|satuan|total|harga|keterangan|entitas|unit|rekap\s*order|periode|outlet|frozen|kering|chinese|traditional|specialist|kode\s*barang|cv\.?\s*selera|pt\.?\s*rasyuka|ciputra|trans\s*icon|delta\s*super|s\.?city|wtc|tandes|grand\s*city|icon\s*gresik|gres\s*mall)/i;
const UNIT_WORDS = /(?:Pack|Ekor|Kg|Box|Pail|Unit|Liter|Pcs|pcs)\b/i;
const OUTLET_NOISE = /\b(CIPUTRA|TRANS\s*ICON|DELTA\s*SUPER|S\.?CITY|WTC|TANDES|GRAND\s*CITY|ICON\s*GRESIK|GRES\s*MALL)\b/gi;

/**
 * Strip NO urut PDF (acuan: /^\d+\s+/ ) + normalisasi nama.
 * Hanya nama barang + total qty yang dipertahankan.
 */
function stripLeadingNoAndNormalize(rawName) {
  let n = String(rawName || '').trim();
  // Acuan A.1: NO urut PDF — angka di kolom pertama
  n = n.replace(/^\d+\s+/, '');
  n = n.replace(/^\d{1,4}[\.\)]\s*/, '');
  // Hapus size/qty di dalam kurung (acuan normalisasi)
  n = n.replace(/\s*\([^)]*\)\s*/g, ' ');
  // Hapus supplier / noise umum
  n = n
    .replace(/\s*\/\s*CV\.?\s*PD3\s*CHICKEN/gi, '')
    .replace(/\s*\/\s*GOOD\s*EAT/gi, '')
    .replace(OUTLET_NOISE, '')
    .replace(/[-–—]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  return n;
...
export function parseLinesFromText(text, preLines) {
  ...
  if (lines.length <= 2 && text && text.length > 80) {
    ...
    // Pecah juga pola "88 Nama Barang" tanpa titik
  ...
  for (const line of lines) {
    ...
    // Skip baris yang hanya berisi angka / qty outlet
    if (/^[\d.\s,|xX×-]+$/.test(line)) continue;

    // Acuan A.1: strip NO urut dari depan
    ...
    if (unitMatch) {
      ...
      // Ambil angka terakhir setelah unit = kolom TOTAL (abaikan qty per outlet)
      ...
      // Fallback: angka trailing sebagai TOTAL qty
