...
const HEADER_RE =
  /^(no\.?|keterangan|nama\s*barang|barang|size|satuan|unit|total|po\s*cv|po\s*pt|tgl|tanggal|outlet|halaman|page|rekap|order|cv\.|pt\.|frozen\s*food|cold\s*storage|periode|area\s*gudang|chinese\s*food|traditional\s*food|selera\s*bogatama|rasyuka|no\s+keterangan)/i;

const SKIP_LINE_RE =
  /^(periode|tanggal|tgl|halaman|page|rekap\s*order|area\s*gudang|chinese\s*food|traditional|selera\s*bogatama|cv\.?\s*selera|pt\.?\s*rasyuka|food\s*specialist|no\s+keterangan\s+unit)/i;

const UNIT_ONLY_RE =
  /^(pack|pcs|ekor|kg|box|pail|unit|liter|porsi|frozen|chilled|dry|btl|botol|bal|dus)$/i;

/** Continuation row: unit/storage words + numbers only (e.g. "Pack FROZEN 20.00 20.00") */
const CONTINUATION_RE =
  /^(?:(?:pack|pcs|pail|ekor|kg|box|unit|frozen|chilled|dry)\s*)+[\d.\s,|]*$/i;
...
export function parseLinesFromText(text) {
  ...
  for (const line of raw) {
    ...
    if (chunks.length >= 2) {
      ...
      for (const c of chunks) {
        ...
        if (joined.length && /\(\s*$/.test(prev) && /^\d/.test(c)) {
          joined[joined.length - 1] = `${prev} ${c}`;
        } else {
          joined.push(c);
        }
      }
  ...
  for (let i = 0; i < expanded.length; i++) {
    ...
    if (/^\d{1,2}\s+(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember)\b/i.test(line)) continue;
    ...
    while (i + 1 < expanded.length) {
      ...
      if (numTail && !HEADER_RE.test(nxt) && !SKIP_LINE_RE.test(nxt)) {
        line = `${line} ${nxt}`;
        i += 1;
        continue;
      }
