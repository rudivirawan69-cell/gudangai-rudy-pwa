...
export function matchByAlias(entity, name) {
  ...
  // Partial alias — prefer longest / most specific (nugget katsu over nugget)
  const aliasHits = [];
  for (const [alias, kode] of Object.entries(lookup)) {
    if (!alias || alias.length < 2) continue;
    const exact = alias === normalized;
    const queryContainsAlias = normalized.includes(alias);
    const aliasContainsQuery = alias.includes(normalized);
    if (exact || queryContainsAlias || aliasContainsQuery) {
      const score = exact
        ? 1000 + alias.length
        : (queryContainsAlias ? 500 + alias.length : 100 + alias.length);
      aliasHits.push({ alias, kode, score, exact });
    }
  }
  if (aliasHits.length) {
  ...
  // Partial nama — prefer longest / most specific
  const namaHits = list
    .map(i => {
      const n = i.nama.toLowerCase();
      if (n === normalized) return { item: i, score: 1000 + n.length };
      if (normalized.includes(n)) return { item: i, score: 500 + n.length };
      if (n.includes(normalized)) return { item: i, score: 100 + n.length };
      return null;
    })
    .filter(Boolean)
