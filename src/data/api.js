 ...
 * + fromQueue guard (tidak enqueue ulang saat sync) + normalizeQueue + sync lock
 * + addTransactionBatch: kirim seluruh keranjang dalam satu request
 */
...
function getQueuedClientItemIds() {
  try {
    ...
    for (const e of queue) {
      ...
      for (const it of e.items || []) {
        if (it.clientItemId) ids.add(it.clientItemId);
      }
    }
    return ids;
  } catch {
...
function enqueue(type, entity, items, meta = {}) {
  if (meta && meta.fromQueue) {
    return { id: null, skipped: true, items: [], reason: 'fromQueue' };
  }
  const queue = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
  const alreadyQueued = getQueuedClientItemIds();
  const pending = (items || [])
    .map(ensureClientItemId)
    .filter((it) => !isApplied(it.clientItemId) && !alreadyQueued.has(it.clientItemId));
...
async function submitBatchItems(sheet, entity, items, tanggal) {
  ...
  const prepared = items.map((it) => {
    ...
    return {
      ...
      kodeBarang: String(item.kode || '').trim(),
      qty: Number.isFinite(n) ? Math.round(n * 1000) / 1000 : 0,
      keterangan: String(item.keterangan || '').trim(),
      requestId: item.clientItemId,
      transactionId: 'TX-' + item.clientItemId,
      nonce: 'NC-' + item.clientItemId,
  ...
  const payload = {
    ...
    items: prepared.map(({ kode, kodeBarang, qty, keterangan, requestId, transactionId, nonce }) => ({
      kode, kodeBarang, qty, keterangan, requestId, transactionId, nonce,
    })),
  };
  ...
  if (data && (data.error === 'UNKNOWN_ACTION' || data.code === 'UNKNOWN_ACTION' ||
      (Array.isArray(data.available) && !data.available.includes('addTransactionBatch')))) {
  ...
  if (Array.isArray(data?.details)) {
    for (const d of data.details) {
      if (d && d.kode) detailByKode[String(d.kode)] = d;
    }
  }

  if (data && (data.success === true || data.status === 'APPLIED')) {
    for (const p of prepared) {
      markApplied(p.clientItemId);
      const d = detailByKode[p.kode] || {};
      written.push({
        success: true,
        kode: p.kode,
        qty: d.qty != null ? d.qty : p.qty,
        row: d.row,
        status: d.status || data.status || 'APPLIED',
        clientItemId: p.clientItemId,
      });
    }
    return { success: true, written, failedItems: [], errors: [], data };
  }

  if (data && Array.isArray(data.details) && data.details.length > 0) {
    const okKodes = new Set(data.details.map((d) => String(d.kode || '')));
    for (const p of prepared) {
      if (okKodes.has(p.kode)) {
        markApplied(p.clientItemId);
        const d = detailByKode[p.kode] || {};
        written.push({
          success: true,
        ...
        failedItems.push(p._orig);
        errors.push(p.kode + ': gagal di batch');
      }
    }
    return {
      success: false,
      ...
      errors,
      data,
      error: data.error || `Sebagian gagal (${written.length}/${prepared.length})`,
    };
  }
  ...
  return {
    success: false,
    written: [],
    failedItems: prepared.map((p) => p._orig),
    errors: [data?.error || data?.code || 'Batch gagal'],
    data,
    error: data?.error || data?.code || 'Batch gagal',
  };
}

async function submitSequential(sheet, entity, todo, tanggal, typeMap, action, fromQueue, alreadyDone) {
  const written = [];
  const errors = [];
  const failedItems = [];
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    try {
      ...
      if (res.unauthorized) {
        const remaining = todo.slice(i).filter((x) => !isApplied(x.clientItemId));
        return {
          success: false,
          offline: false,
      ...
      if (isNetwork) {
        ...
        return {
          ...
          remaining: rest,
          written: written.length + alreadyDone,
          error: 'Jaringan terputus — sisa ' + rest.length + ' item menunggu sync berikutnya.',
          details: written,
        };
      }
  ...
  if (failedItems.length === 0) {
    return { success: true, offline: false, written: totalDone, skipped: alreadyDone, remaining: [], details: written };
  }

  if (stillPending.length && !fromQueue) {
    enqueue(typeMap[action], entity, stillPending, { tanggal });
  }
  ...
  return {
    ...
    error:
      'Sebagian gagal (' + totalDone + '/' + (todo.length + alreadyDone) + '). ' +
      errors.join('; ') +
      (stillPending.length ? ' · ' + stillPending.length + ' item menunggu.' : ''),
    details: written,
  };
}
...
async function submitTransaction(action, entity, items, options = {}) {
  ...
  if (!url) {
    ...
    return {
      ...
      offline: true,
      id: entry.id,
      remaining: (items || []).map(ensureClientItemId).filter((it) => !isApplied(it.clientItemId)),
  ...
  try {
    const batchRes = await submitBatchItems(sheet, entity, todo, tanggal);

    if (batchRes.unauthorized) {
      return {
        success: false,
        offline: false,
        written: alreadyDone,
    ...
    if (batchRes.fallback) {
      return submitSequential(sheet, entity, todo, tanggal, typeMap, action, fromQueue, alreadyDone);
    }
    ...
    if (batchRes.success) {
      return {
        success: true,
        offline: false,
        written: (batchRes.written?.length || 0) + alreadyDone,
        skipped: alreadyDone,
        remaining: [],
        details: batchRes.written || [],
        batch: true,
      };
    }

    const stillPending = (batchRes.failedItems || []).filter((it) => !isApplied(it.clientItemId));
    if (stillPending.length && !fromQueue) {
      enqueue(typeMap[action], entity, stillPending, { tanggal });
    }
    return {
      ...
      written: (batchRes.written?.length || 0) + alreadyDone,
      remaining: stillPending,
      error: batchRes.error || 'Batch sebagian/gagal',
      details: batchRes.written || [],
      batch: true,
    };
  } catch (err) {
    ...
    if (isNetwork) {
      if (!fromQueue) {
        const entry = enqueue(typeMap[action], entity, todo, { tanggal });
        return {
          success: alreadyDone > 0,
          offline: true,
          id: entry.id,
          remaining: todo,
          written: alreadyDone,
          error: 'Jaringan terputus — ' + todo.length + ' item di antrian (tanpa dobel).',
          details: [],
        };
      }
      return {
        ...
        remaining: todo,
        written: alreadyDone,
        error: 'Jaringan terputus — sisa ' + todo.length + ' item menunggu sync berikutnya.',
        details: [],
      };
    }
    return submitSequential(sheet, entity, todo, tanggal, typeMap, action, fromQueue, alreadyDone);
  ...
  try {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        ...
        const still =
          Array.isArray(res.remaining) && res.remaining.length
            ? res.remaining.filter((it) => !isApplied(it.clientItemId))
            : pendingItems.filter((it) => !isApplied(it.clientItemId));
    ...
    const cleaned = queue.filter((e) => !e.synced && (e.items || []).length > 0);
