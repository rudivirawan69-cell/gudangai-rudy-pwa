...
/** Export: tandai clientItemId sudah sukses (anti re-enqueue). */
...
async function submitTransaction(action, entity, items, options = {}) {
  ...
  if (!fromQueue && todo.length >= 1) {
    // Chunked write-once: pecah batch besar jadi potongan kecil agar tiap POST
    // selesai dalam WRITE_TIMEOUT_MS. Sequential, idempotent, anti-dobel.
    // Partial success = item yang sudah COMMITTED ditandai applied; sisa masuk antrian.
    const chunks = [];
    for (let i = 0; i < todo.length; i += BATCH_CHUNK_SIZE) {
      chunks.push(todo.slice(i, i + BATCH_CHUNK_SIZE));
    }
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      try {
        ...
        console.warn('[GudangAI] Chunk ' + (ci + 1) + '/' + chunks.length + ' response not clear, enqueue:', batchRes);
        enqueue(typeMap[action], entity, chunk, { tanggal });
        remainingAll.push(...chunk);
      } catch (batchErr) {
        console.warn('[GudangAI] Chunk ' + (ci + 1) + '/' + chunks.length + ' timeout/fail — enqueue (no serial):', batchErr?.message || batchErr);
        enqueue(typeMap[action], entity, chunk, { tanggal });
        remainingAll.push(...chunk);
      }
    }
    if (unauthorized) {
      return { success: false, error: 'Unauthorized — isi API Secret di Atur', remaining: remainingAll.length ? remainingAll : todo, written: totalWritten };
    }
...
export function removePendingByClientIds(ids) {
  ...
  try {
    ...
    const next = q.map(e => ({ ...e, items: (e.items || []).filter(it => !set.has(it.clientItemId)) }));
    localStorage.setItem(QUEUE_KEY, JSON.stringify(next.filter(e => (e.items || []).length)));
  } catch {}
}
