...
export async function healthCheck() {
  ...
  try {
    ...
    if (data && (data.success === true || data.status === 'OK' || data.status === 'ok')) {
      return { ok: true, data: { status: 'ok', version: data.version || data.title || 'V6.5.4', timestamp: data.serverTime || new Date().toISOString(), spreadsheet: data.spreadsheet || '', raw: data } };
    }
...
export async function getStatusPO() {
  try {
    ...
    if (!data || data.code === 'UNAUTHORIZED' || (data.success === false && !data.summary && !data.items)) {
      try {
        data = await postJson({ action: 'getStatusPO', requestId: newIds().requestId });
      } catch (e2) {
        if (data) return data;
...
async function submitTransaction(action, entity, items, options = {}) {
  ...
  if (!fromQueue && todo.length >= 1) {
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      try {
        ...
        const transactions = chunk.map((it) => {
          ...
          return { sheet, entitas: entity, kodeBarang: String(it.kode || '').trim(), qty: Number(it.qty) || 0, keterangan: String(it.keterangan || '').trim(), tanggal: tanggal || undefined, requestId: cid, transactionId: 'TX-' + cid, nonce: 'NC-' + cid };
        });
    ...
    if (unauthorized) return { success: false, error: 'Unauthorized — isi API Secret di Atur', remaining: remainingAll.length ? remainingAll : todo, written: totalWritten };
    if (remainingAll.length === 0) {
      try { window.dispatchEvent(new CustomEvent('gudangai-stock-refresh')); } catch (_) {}
      return { success: true, written: totalWritten, skipped: alreadyDone, remaining: [], message: 'Semua item berhasil ditulis (chunked).' };
    }
    const queuedN = remainingAll.length;
    const okN = totalWritten - alreadyDone;
    pushNotification({
      type: okN > 0 ? 'ok' : 'warn',
      title: okN > 0 ? ('Berhasil ' + okN + ' · antrian ' + queuedN) : 'Verifikasi antrian',
      body: okN > 0
        ? (okN + ' item sudah di sheet. ' + queuedN + ' item di Antrian — cek spreadsheet; jika sudah tertulis tekan Sukses (jangan kirim ulang).')
        : (queuedN + ' item masuk Antrian (timeout). Server mungkin sudah menulis. Cek spreadsheet — jika sudah, tekan Sukses.'),
    });
    return {
      success: okN > 0,
      ...
      error: okN > 0
        ? (okN + ' berhasil · ' + queuedN + ' di antrian verifikasi. Cek sheet; tekan Sukses jika sudah tertulis.')
        : 'Respons timeout. Item di Antrian. Cek spreadsheet — jika sudah ada, tekan Sukses (jangan kirim ulang).',
    };
  }
...
export async function syncPendingQueue() {
  if (_syncLock) return { synced: 0, failed: 0, skipped: true };
  if (!getApiUrl() || !navigator.onLine) return { synced: 0, failed: 0, skipped: true };
  _syncLock = true;
  try {
    ...
    for (const entry of queue) {
      ...
      const action = entry.type === 'masuk' ? 'barangMasuk' : entry.type === 'rusak' ? 'barangRusak' : 'barangKeluar';
      try {
        const res = await submitTransaction(action, entry.entity, entry.items || [], { tanggal: entry.tanggal, fromQueue: true });
        if (res.success && !(res.remaining || []).length) {
          entry.synced = true;
          (entry.items || []).forEach((it) => markApplied(it.clientItemId));
          synced += (entry.items || []).length;
        } else if (res.unauthorized) { failed++; break; }
        else {
          const rem = res.remaining || entry.items || [];
          entry.items = rem;
          if (!rem.length) { entry.synced = true; synced += 1; }
          else failed++;
...
export function pushNotification(n) {
  try {
    ...
    list.unshift({ id: 'N-' + Date.now(), ...n, at: new Date().toISOString(), read: false });
...
export async function generatePO(payload) {
  try {
    const data = await postJsonWrite({ action: 'generatePO', ...payload });
...
export async function submitPO(payload) {
  try {
    const data = await postJsonWrite({ action: 'submitPO', ...payload });
