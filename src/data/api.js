 ...
 * + fromQueue guard + normalizeQueue + sync lock
 * Enhanced for Backend V6.4.4 + STATUS-PO
 */
...
/** Default backend V6.4.4 + STATUS-PO */
...
export function getApiUrl() {
  ...
  if (!localStorage.getItem('gudangai_api_url_seeded')) {
...
export async function healthCheck() {
  ...
  try {
    ...
    if (!data || (data.error && !data.success && data.status !== 'OK' && data.code !== 'UNAUTHORIZED')) {
      try { data = await postJson({ action: 'ping', requestId: newIds().requestId }); }
      catch (e) { lastErr = e; }
    }
    ...
    if (data && (data.success === true || data.status === 'OK' || data.status === 'ok')) {
      return {
        ...
        data: {
          status: 'ok',
          version: data.version || data.app || data.title || 'V6.4.4',
...
/** Dashboard gabungan (PO + low stock) */
export async function getDashboardData() {
  const url = getApiUrl();
  if (!url) return { success: false, error: 'URL API belum diatur' };
  try {
    let data;
    try { data = await getJson('getDashboardData'); }
    catch {
      try { data = await getJson('getDashboard'); }
      catch {
        data = await postJson({ action: 'getDashboardData', requestId: newIds().requestId });
      }
    }
    if (data && data.code === 'UNAUTHORIZED') {
      return { success: false, error: 'Unauthorized — isi API Secret di Atur' };
    ...
    return { success: false, error: err.message || 'Gagal mengambil dashboard' };
  }
}
...
async function submitOneItem(sheet, entity, it, tanggal) {
  ...
  if (data && (data.status === 'DUPLICATE' || data.idempotent === true || data.idempotentDuplicate === true)) {
    markApplied(cid);
    return { success: true, skipped: true, kode: item.kode, clientItemId: cid, row: data.row };
  }
  if (data && (data.success === true || data.status === 'COMPLETED' || data.status === 'APPLIED' || data.status === 'OK')) {
    markApplied(cid);
    return {
      success: true,
      ...
      status: data.status,
      clientItemId: cid,
      shortageHold: data.shortageHold,
    };
  }
  return {
    success: false,
