...
async function submitTransaction(action, entity, items, options = {}) {
  ...
  if (!fromQueue && todo.length >= 1) {
    try {
      const batchId = 'BATCH-' + (todo[0].clientItemId || Date.now());
      const transactions = todo.map((it) => {
      ...
      if (batchRes && (batchRes.success === true || batchRes.status === 'COMMITTED' || batchRes.status === 'APPLIED') && !batchRes.error) {
        ...
        const writtenN = Number(batchRes.successCount) || todo.length;
        return { success: true, written: writtenN + alreadyDone, skipped: alreadyDone, remaining: [], details: batchRes.results || [] };
      }
