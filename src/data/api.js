...
async function fetchWithRetry(url, options = {}, retries = RETRY_COUNT, timeoutMs = REQUEST_TIMEOUT_MS, emitFailure = true) {
  ...
  // Write-path failures must NOT flip the global connection banner to "disconnect".
  // Long batch POST (90s) can time out while the network remains online.
  if (emitFailure) {
    emitConn({ state: 'failed', error: (lastError && lastError.message) || 'Gagal' });
  } else {
    emitConn({ state: 'write_unknown', error: (lastError && lastError.message) || 'Respons tulis belum jelas' });
  }
  throw lastError;
}
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    try {
      ...
      if (isNetwork) {
        ...
        return { success: written.length > 0, partial: true, remaining: rest, written: written.length + alreadyDone, error: 'Jaringan terputus — sisa antrian.' };
      }
...
export function removePendingByClientIds(clientIds) {
  ...
  return { remaining: next.reduce((n, e) => n + (e.items || []).length, 0) };
}
export function pushNotification(n) {
  try {
    const list = JSON.parse(localStorage.getItem(NOTIF_KEY) || '[]');
    list.unshift({ id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6), read: false, ts: new Date().toISOString(), ...n });
    if (list.length > 40) list.length = 40;
    localStorage.setItem(NOTIF_KEY, JSON.stringify(list));
    try { window.dispatchEvent(new CustomEvent('gudangai-notif')); } catch (_) {}
  } catch (_) {}
}
...
export function unreadNotificationCount() {
  try { return JSON.parse(localStorage.getItem(NOTIF_KEY) || '[]').filter((n) => !n.read).length; } catch { return 0; }
}
export async function submitPO(payload) {
  try {
    ...
    return { success: false, error: err?.message || 'Gagal submit PO' };
  }
}
export async function generatePO(tipe) {
  try {
    const data = await postJsonWrite({ action: 'generatePO', tipe });
    return data || { success: false, error: 'Respons kosong' };
  } catch (err) {
