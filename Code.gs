...
function route_(action, params, body) {
  ...
  // Stock endpoints (V6.4.15 contract)
  ...
  // Transaction write (anti-double-submit preserved)
  ...
  if (a === 'getTransactionStatus' || a === 'getTransactionHistory') {
    return json_({ success: true, status: 'OK', message: 'History endpoint available; use spreadsheet for full history', version: VERSION });
  }
  ...
  if (a === 'runPOAnalysis' || a === 'getPOAnalysis' || a === 'writePORecommendations') {
    return json_({ success: true, version: VERSION, message: 'PO analysis uses live spreadsheet data (dynamic CS)', stats: getDynamicMasterStats_() });
  }
  ...
  return json_({
    ...
    version: VERSION,
    available: [
      'status', 'ping', 'healthCheck', 'bootstrap', 'getConfig',
      'getAllStock', 'getStock', 'getStockByCode', 'getLowStock',
      'addTransaction', 'addTransactionBatch', 'getTransactionStatus', 'getTransactionHistory',
      'searchByName',
      'generatePO', 'writePurchaseOrder', 'submitPO', 'sendPO',
      'getStatusPO', 'getDashboardData', 'getDashboard', 'refreshDashboard', 'refreshDashboardPO',
      'runPOAnalysis', 'getPOAnalysis', 'writePORecommendations',
      'clearMasterCache', 'maintenanceCleanup'
    ]
  });
}
...
/** Dynamic master statistics — runtime only, never hard-coded counts */
function getDynamicMasterStats_() {
  var cv = getAllStock_('CV');
  var pt = getAllStock_('PT');
  var combined = cv.concat(pt);
  var csCount = 0;
  for (var i = 0; i < combined.length; i++) {
    if (String(combined[i].divisi || '').toUpperCase() === 'CS') csCount++;
  }
  return {
    ...
    pt: pt.length,
    cs: csCount,
    source: 'runtime-spreadsheet',
    note: 'Counts derived from Stock CV + Stock PT at call time. Not constants.'
  };
}

function getAllStockCombined_() {
  var cv = getAllStock_('CV');
  var pt = getAllStock_('PT');
  return cv.concat(pt);
}

/** PO CS items: filter by divisi == "CS" at runtime (no hard-coded 43) */
function getCSItemsForPO_() {
  var all = getAllStockCombined_();
  var cs = [];
  for (var i = 0; i < all.length; i++) {
    if (String(all[i].divisi || '').toUpperCase() === 'CS') cs.push(all[i]);
  }
  if (cs.length === 0) {
    throw new Error('Tidak ditemukan item divisi CS pada spreadsheet. Periksa kolom Divisi di Stock CV / Stock PT.');
  }
  return cs;
}
...
function writePurchaseOrder_(body) {
  // PO uses runtime CS items (divisi == 'CS'); never hard-coded count
  var res = { success: false, error: 'Gunakan PO dari PWA — endpoint tersedia', version: VERSION };
  try {
    var cs = getCSItemsForPO_();
    res.csItemCount = cs.length;
    res.note = 'CS items resolved dynamically: ' + cs.length;
  } catch (e) {
    res.csError = String(e.message || e);
  }
  try {
